...
| Packaging: manifest-driven extraction end-to-end over a committed dump living OUTSIDE `tests/f90` — the quickstart toy pair (`examples/quickstart/toy_kernel_ptree`, with its own `PROVENANCE` mirroring this directory's convention) | `examples/quickstart/toy_kernel.f90` | `kernel_bank.load_manifest`/`render_fortran`, `flang_kernel.FlangKernelFrontend`; pinned by `tests/test_kernel_bank.py` (`TestQuickstart`) |
