...
/-- The C++ port computes the same function as the Fortran subroutine, on
every input. `rfl` means the two generated definitions are definitionally
equal — Lean's kernel sees them as the same function by unfolding alone. -/
