...
`kir.pointize` performs Track B's central semantic move: it strips a kernel's
...
per-point function — the same shape as a TIM/AMReX device kernel, which is
...
declares that the iterations may execute in any order. Track B takes the
...
assertion the source never made — so Track B doesn't assert it; it **proves**
it, once and for all, as a schema lemma in `lean/pilot/Pilot/SeqSchema.lean`:
...
For `do concurrent`, Track B accepts an **assertion** (the source's own).
For plain `do`, it supplies a **proof**. Plain DO ends up on equal — arguably
...
Refusal by Lean: loud, and never a wrong model. (This is recorded in
`kir.py`'s docstring as part of the model's meaning.)
