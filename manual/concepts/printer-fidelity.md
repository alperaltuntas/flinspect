...
- **Literals keep their spelling**, normalized only to Lean's numeral syntax:
  `3.0` prints as `3`, `0.5` as `0.5`. The Fortran frontend reads the
  *structured* literal from the dump tree (never the unparse text, where
  `12.0` can resurface as `1.2e1_8`), and clang's shortest-round-trip
  `FloatingLiteral` values land on the same numerals from the other side.
